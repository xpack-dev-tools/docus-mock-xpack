...
`C:\\>%USERPROFILE%\\AppData\\Roaming\\xPacks\\@xpack-dev-tools\\docus-mock\\${ props.version }-${ props.xpack_subversion }.1\\.content\\bin\\docus-mock.exe --version
${ props.version }
`}</code></pre>
...
<ul>
<li><code>xpack-docus-mock-{ props.version }-{ props.xpack_subversion }-win32-x64.zip</code></li>
</ul>
...
<pre><code className="language-console">{
`C:\\>%USERPROFILE%\\AppData\\Roaming\\xPacks\\docus-mock\\xpack-docus-mock-${ props.version }-${ props.xpack_subversion }\\bin\\docus-mock.exe --version
${ props.version }
`}</code></pre>
